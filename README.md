...
## 👽️ HamNoSys
You can use [HamNoSys notation parsing tool](https://github.com/hearai/parse-hamnosys), that was created in parallel with the pipeline to convert HamNoSys notation to classes required by the pipeline (```"test_hamnosys.txt"```). Keep in mind, that this tool analyzes sign initial position only (movement description analysis is not implemented yet).

For a file generated by the tool, you can create basic charts with statistics. Charts contain information on how many times each HamNoSys sign in a specific category appears in the dataset.
